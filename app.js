  ...
  new FacebookStrategy(
    {
      ...
      callbackURL:
        process.env.LOGIN_CALLBACK_URL + "/auth/facebook/facebooklogin",
      profileFields: [
        "id",
        "displayName",
        "email",
        ...
        function (err, foundLab) {
          if (err) {
            ...
            if (foundLab) {
              ...
              if (
                (new Date(foundLab[0].startDateAndTime) <=
                  new Date(currentDateAndTime) &&
                  new Date(foundLab[0].endDateAndTime) >=
                    new Date(currentDateAndTime)) ||
    ...
    if (accountStatus != "active") {
      ...
      res.render("login", {
        ...
        errors: [
          {
            message:
              "Cannot log in, you account is in " + accountStatus + " state!",
          },
        ],
        user: req.user,
        tempUserData: null,
      ...
      if (err) {
        ...
        if (!foundUser) {
          ...
          if (foundUser.accountStatus === "pending") {
            ...
            errors.push({
              message:
                "Cannot login, your account is in " +
            ...
            if (foundUser) {
              ...
              token.save(function (err) {
                if (err) {
                  ...
                  const content = generateResetEmailBody(
                    foundUser.local.fullname,
                    process.env.LOGIN_CALLBACK_URL +
                      "/resetPassword/" +
                      token.token +
                      "/" +
        ...
        function (err, foundToken) {
          if (err) {
            ...
            if (foundToken) {
              ...
              if (
                (req.body.new_password && req.body.new_password.length < 8) ||
                (req.body.confirm_password &&
      ...
      fs.readdir(__dirname + "/public/share/uploads", function (err, files) {
        ...
        files.forEach(function (file) {
          const mydate = new Date(
            fs.statSync(__dirname + "/public/share/uploads/" + file).mtimeMs
          );
          const month = [
            "January",
            "February",
      ...
      User.findById(req.user.id, function (err, foundUser) {
        if (err) {
          ...
          if (foundUser) {
            ...
            foundUser.githubURL =
              req.body.profile_githubUsername.trim() == ""
                ? ""
                : parseGithubUrl(req.body.profile_githubUsername).path;
...
app.post("/changePassword", function (req, res) {
  ...
  if (req.isAuthenticated()) {
    if (req.user.accountStatus === "pending") {
      ...
      if (
        !profile_currentPassword ||
        !profile_newPassword ||
        !profile_confirmPassword ||
        profile_currentPassword.trim() === "" ||
        profile_newPassword.trim() === "" ||
        profile_confirmPassword.trim() === ""
      ) {
        errors.push({
          message: "Please enter all fields correctly!",
        });
      ...
      if (
        (profile_newPassword && profile_newPassword.length < 8) ||
        (profile_confirmPassword && profile_confirmPassword.length < 8)
      ) {
        errors.push({
          message: "Password should be 8-20 characters long!",
        });
    ...
    if (req.user.accountStatus === "pending") {
      ...
      if (req.user.role != "superuser") {
        ...
        if (req.params.setting === "manageUsers") {
          ...
          fs.readdir(
            __dirname + "/public/share/uploads",
            function (err, files) {
              if (err) {
                return console.log("Unable to scan directory: " + err);
              }
              ...
              files.forEach(function (file) {
                const mydate = new Date(
                  fs.statSync(
                    __dirname + "/public/share/uploads/" + file
                  ).mtimeMs
                );
                const month = [
                  "January",
...
app.post("/deleteFile", function (req, res) {
  fs.unlink(
    __dirname + "/public/share/uploads/" + req.body.fileName,
    function (err) {
      if (err) throw err;
    }
  );
